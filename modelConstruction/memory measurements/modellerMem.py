...
issueTiming = "mul r0, r1, r1 "   #MLA instruction seems unable to execute in parallel with previous instructions (wrong)
latencyTiming = "mul r0, r1, r2 "
writebackTiming = "mul r0, r1, sp "
...
def switchplusminus(insn):
    insn = insn.group(0)
    insn = insn.replace("+","(TOKEN)")
    insn = insn.replace("-","+")
    insn = insn.replace("(TOKEN)","-")
    return insn
    ...
    for format in casm.allInstructions:
        for insn in casm.Formatted(format):
            ...
            for aux in testString(insn[0], insn[1]):
                ...
                issAsmString = " %s; nop; nop; %s; %s; %s; nop; nop;" % (aux, issueTiming, insn[0], issueTiming) #todo: add the correct surrounding code
                latAsmString = " %s; nop; nop; %s; %s; %s; nop; nop;" % (aux, latencyTiming, insn[0], latencyTiming) #todo: add the correct surrounding code
                ...
                if "!" in format:
                    wrbackAsmString = " %s; nop; nop; %s; %s; %s; %s; nop; nop;" % (aux, writebackTiming, insn[0], writebackTiming, re.sub(",[^\]]",switchplusminus,insn[0]), writebackTiming) 
                    wrbackprogramcode = createTestProgramCode(wrbackAsmString)
                    wrbacktiming = measure(wrbackprogramcode)
                    if (type(wrbacktiming) != float):
                        notes = "%s; wrback timing error: %s" % (notes, wrbacktiming)
                        wrbacktiming = -1
                    else:
                        wrbacktiming = (wrbacktiming - 1 ) / 2
                        notes = "%s; wrbackTiming:: %0.1f ::" % (notes, wrbacktiming)
    ...
    def next(self):
        if self.i == self.maxI:
            ...
            if (self.maxI == 2): #there's a condition code
                if self.i == 0:
                    result = auxiliaries[self.cc][0]
                else:
                    result = auxiliaries[self.cc][1]
            else:
                result = auxiliaries[""] #this is the non-conditional scenario
...
int funl() {
	...
	__asm__ __volatile__ (
	    asminst("ldr r12, [sp]; ldr r10, [sp,#1]; ldr r8, [sp, #2]; mov r9, #1; ldr r6, [sp,#-1]; ldr r7, [sp, #-2];", \""""

programEnd = """", "str r12, [sp]; str r10, [sp,#1]; str r8, [sp, #2]; str r6, [sp,#-1]; str r7, [sp, #-2];")
	    : :  : "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r7", "r8", "r9", "r10", "r12", "sp");
