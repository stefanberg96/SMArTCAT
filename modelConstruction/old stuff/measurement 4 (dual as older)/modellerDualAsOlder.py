...
issueTiming = "mov r3, #1; mla r1, r3, r3, r3; "   #MLA instruction seems unable to execute in parallel with previous instructions (wrong)
latencyTiming = "mov r3, #1; mla r1, r3, r3, r3; "
    ...
    for format in casm.allInstructions:
        for insn in casm.Formatted(format):
            ...
            for asmString in testString(insn[0], insn[1]):
                ...
                issAsmString = "%s; %s; %s" % (insn[0], asmString, issueTiming)
    ...
    def next(self):
        if self.i == self.maxI:
            ...
            if (self.maxI == 2): #there's a condition code
                if self.i == 0:
                    result = "%s; mul r1, r2, r3; %s" % (auxiliaries[self.cc][0], result)
                else:
                    result = "%s; mul r1, r2, r3; %s" % (auxiliaries[self.cc][1], result)
            else:
