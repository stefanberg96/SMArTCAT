...
def model():
    ...
    asm = "%s, %s" % (asm, (lfsr % (counter, counter)))
    programcode = createTestProgramCode(asm)
    timing = measure(programcode)
    print "result: %d" % timing
    timing = measure(programcode)
    print "result: %d" % timing
    timing = measure(programcode)
    print "result: %d" % timing
...
int funl() {
	...
	__asm__ __volatile__ (
	    asminst("mov r3, #200; sub r3, #1; ldr r0, [fp, #-128]; eor %0, r0, r3;", \""""
